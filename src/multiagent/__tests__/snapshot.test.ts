...
import { SNAPSHOT_SCHEMA_VERSION } from '../../agent/snapshot.js'
...
describe('multiagent snapshot', () => {
  ...
  describe('takeSnapshot', () => {
    ...
    it('serializes MultiAgentState via toJSON', () => {
      const graph = makeGraph('g', ['a'])
  ...
  describe('loadSnapshot', () => {
    it('restores MultiAgentState from snapshot', () => {
      ...
      const snapshot = takeSnapshot(graph, state)
      const restored = loadSnapshot(graph, snapshot)
    ...
    it('returns undefined when snapshot has no state', () => {
      const graph = makeGraph('g', ['a'])
    ...
    it('restores agent node snapshots (full preset)', () => {
      ...
      agent.state.set('agentKey', 'agentVal')
      ...
      expect(agent.state.get('agentKey')).toBe('agentVal')
    })
    ...
    it('recursively restores nested MultiAgentNode snapshots', () => {
      ...
      innerAgent.state.set('innerKey', 'innerVal')

      const snapshot = takeSnapshot(outer, makeState(['a', 'inner']), { preset: 'full' })

      innerAgent.state.clear()
      loadSnapshot(outer, snapshot)

      expect(innerAgent.state.get('innerKey')).toBe('innerVal')
    })
    ...
    it('works with Swarm orchestrator', () => {
      ...
      const restored = loadSnapshot(swarm, snapshot)

      expect(restored).toBeDefined()
      expect(restored!.steps).toBe(2)
    })

    it('returns undefined when state is null in snapshot data', () => {
      const graph = makeGraph('g', ['a'])
      const snapshot: Snapshot = {
        scope: 'multiAgent',
        schemaVersion: SNAPSHOT_SCHEMA_VERSION,
  ...
  describe('round-trip', () => {
    ...
    it('snapshot survives JSON.stringify/JSON.parse round-trip', () => {
      ...
      const restored = loadSnapshot(graph, parsed)!
    ...
    it('full preset round-trip with nested graph preserves inner agent state', () => {
      ...
      const innerAgent = getAgent(inner, 'x')
      innerAgent.state.set('deep', 'value')
      ...
      expect(innerAgent.state.get('deep')).toBe('value')
    })
  })
})
